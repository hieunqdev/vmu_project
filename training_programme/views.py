...
class View_training_programme(View):
    def get(self, request):
        specializations = Specialization.objects.raw("SELECT training_programme_specialization.id, training_programme_specialization.name AS nganh, training_programme_specialization.studies AS ma_nganh, CASE WHEN (training_programme_specializationclass.name LIKE ('ThS')) THEN 'Thạc sĩ' ELSE 'Tiến sĩ' END AS trinh_do FROM training_programme_specialization INNER JOIN training_programme_specializationclass_specialization ON training_programme_specialization.id = training_programme_specializationclass_specialization.specializationclass_id INNER JOIN training_programme_specializationclass ON training_programme_specializationclass_specialization.specialization_id = training_programme_specializationclass.id;")
        context = {
